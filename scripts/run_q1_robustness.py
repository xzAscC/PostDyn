...
def _pool(args: argparse.Namespace, n: int) -> DomainPool:
    if args.scale == "tiny":
        return q1._tiny_pool(args.domain, n)
    ...
    with tee_log(root):
        ...
        for repeat in range(args.repeats):
            ...
            if path.is_file():
                ...
                continue
            indices = random.Random(42 + repeat).sample(range(pool.actual_n), n)
            records = [pool.records[index].prompt for index in indices]
            values_by_layer: dict[str, list[float]] = {}
            ranks: dict[str, float] = {}
            stability: dict[str, dict[str, float]] = {}
            for layer in layers:
                hidden = extract_layer_hiddens(
                    model, tokenizer, records, [layer], args.batch_size, args.max_length
                )[layer]
