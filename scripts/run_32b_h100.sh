...
run_stage q1_full $RUN uv run python scripts/run_q1.py --family 32b --scale full \
    --dtype bfloat16 --device cuda --sft-lr "$SFT_LR" --output logs/q1/32b \
    "${SHORT_POOL_FLAG[@]}" "${UPLOAD_FLAG[@]}"
run_stage q1_robustness $RUN uv run python scripts/run_q1_robustness.py --family 32b \
    --repeats "$REPEATS" --dtype bfloat16 --device cuda --sft-lr "$SFT_LR" \
    --output logs/q1_robustness/32b "${UPLOAD_FLAG[@]}"
run_stage q2_rlvr $RUN uv run python scripts/run_q2_model.py --family 32b \
    --q1-root logs/q1/32b --model rlvr --sft-lr "$SFT_LR" --output-root logs/q2/32b \
    "${UPLOAD_FLAG[@]}"
run_stage q2_sft $RUN uv run python scripts/run_q2_model.py --family 32b \
    --q1-root logs/q1/32b --model sft --sft-lr "$SFT_LR" --output-root logs/q2/32b \
    "${UPLOAD_FLAG[@]}"

if [ "${DRY_RUN:-0}" = "1" ]; then
    echo "=== H100 32B pipeline complete (dry-run) ==="
else
    echo "=== H100 32B pipeline complete $(date -Is) ===" | tee -a "$LOGFILE"
fi
